...
*   **物理层 (Physical Layer)**: 真实的铁路集装箱中心站，包括轨道、堆场、龙门吊、集卡、列车、集装箱、作业人员等实体及其运行状态。
*   **数据采集层 (Data Acquisition Layer)**: 负责从物理世界获取实时数据，包括但不限于：
    *   **传感器数据**: 龙门吊位置、速度、载荷；集卡位置、速度；列车位置、速度、加速度；集装箱识别 (RFID/OCR)；环境数据等。
...
*   **GET /health**: 获取 API 健康状态。
    *   **响应体示例：**
        ...
        ```
*   **GET /entities**: 查询所有实体的当前状态。
*   **GET /entities/{id}**: 查询特定实体的详细信息。
*   **POST /dispatch/train**: 发送列车调度指令。
    *   **请求体示例：**
        ```json
        {
            "train_id": "train_01",
            "action": "set_target_speed",
            "speed": 40.0
        }
        ```
*   **POST /dispatch/crane**: 发送龙门吊调度指令。
    *   **请求体示例：**
        ```json
        {
            "crane_id": "crane_01",
            "action": "move_to",
            "target_position": {"x": 200.0, "y": 100.0, "z": 15.0}
        }
        ```
...
*   **实体映射**: 将数字孪生模型中的龙门吊、集卡、列车、集装箱等实体映射到 UE5 中的对应资产。
*   **实时数据绑定**: 通过 WebSocket 客户端接收实时数据，更新 UE5 中实体的位置、状态、动画等。
*   **交互功能**: 用户可以在 UE5 中进行视角的切换、信息查询，甚至通过 API 发送模拟指令。

## 6. 技术栈展望

*   **后端/模拟层**: Python (FastAPI for API/WebSocket), C++ (高性能仿真模块), Kafka (消息队列), PostgreSQL/InfluxDB (数据存储)。
